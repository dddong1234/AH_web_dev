...
- 마지막 Conv layer 기준 Grad-CAM heatmap 생성
- 폐렴 여부, 신뢰도, heatmap URL, 모델명 및 예측 수행 일시 반환
...
- 모델 추론 담당자는 마지막 Conv layer를 기준으로 Grad-CAM heatmap을 생성한다.
- 추론 함수는 `record_id`를 받아 heatmap 파일을 지정 경로에 저장하고 응답용 `heatmap_url`을 반환한다.
- API Service는 반환된 URL을 DB 상대 경로로 변환하여 기록하고, DB 저장 실패 시 생성된 파일을 정리한다.
- heatmap 생성에 실패하면 예측 결과를 저장하지 않고 `500 HEATMAP_GENERATION_FAILED`를 반환한다.
- heatmap 파일 저장에 실패하면 예측 결과를 저장하지 않고 `500 HEATMAP_STORAGE_FAILED`를 반환한다.

### 5.6 Heatmap 파일 저장 규칙

| 구분 | 값 |
| --- | --- |
| 로컬 저장 경로 | `uploads/heatmaps/{record_id}/{uuid}.png` |
| DB 저장값 | `heatmaps/{record_id}/{uuid}.png` |
| API 응답 URL | `/uploads/heatmaps/{record_id}/{uuid}.png` |

처리 순서:

1. 추론 함수가 `record_id`를 이용해 `uploads/heatmaps/{record_id}` 디렉터리를 생성한다.
2. UUID 기반 파일명으로 Grad-CAM PNG를 저장한다.
3. 추론 함수는 `/uploads/heatmaps/{record_id}/{uuid}.png`를 `heatmap_url`로 반환한다.
4. API Service는 `/uploads/` 접두사를 제거한 `heatmaps/{record_id}/{uuid}.png`를 DB에 기록한다.
5. API 응답에서는 `/uploads/heatmaps/{record_id}/{uuid}.png` URL을 반환한다.
6. DB 저장 실패 시 API Service가 생성된 heatmap 파일을 삭제한다.
...
| `heatmap_url` | varchar(255) | required, DB 상대 경로 저장 |
...
def predict_pneumonia(image_path: str | Path, record_id: int) -> dict:
    """
...
| `heatmap_url` | string | 저장된 Grad-CAM의 `/uploads/` 응답 URL |
...
추론 함수가 반환한 `heatmap_url`에서는 `/uploads/` 접두사를 제거한 상대 경로를 DB에 저장한다.
...
| `heatmap_url` | string | `/uploads/`로 시작하는 Grad-CAM heatmap 이미지 URL |
...
- `500 HEATMAP_GENERATION_FAILED`
- `500 HEATMAP_STORAGE_FAILED`
- `504 REQUEST_TIMEOUT`
...
| Grad-CAM 생성 실패 | `500` | `HEATMAP_GENERATION_FAILED` |
| Heatmap 파일 저장 실패 | `500` | `HEATMAP_STORAGE_FAILED` |
...
7. 기존 결과를 재사용할 때는 Grad-CAM을 다시 생성하지 않는다.
8. DB의 `heatmap_url` 앞에 `/uploads/`를 붙여 API 응답 URL로 변환한다.
...
- 모델 추론은 `asyncio.to_thread()`로 실행한다.
- 예측 API 전체 처리에 3초 timeout을 적용한다.
- 요청 수신부터 응답 생성까지 3초를 초과하면 `504 REQUEST_TIMEOUT`을 반환한다.
...
| 정상 X-ray Grad-CAM | heatmap PNG 생성 및 URL 반환 |
| 폐렴 X-ray Grad-CAM | heatmap PNG 생성 및 URL 반환 |
| heatmap 파일 저장 | 로컬·DB·응답 경로가 저장 규칙과 일치 |
| 동일 `record_id + ai_model` 재요청 | 추론 함수를 호출하지 않고 저장 결과 반환 |
| 기존 결과 heatmap 재사용 | Grad-CAM을 다시 생성하지 않고 기존 URL 반환 |
...
| Grad-CAM 생성 실패 | `500 HEATMAP_GENERATION_FAILED` |
| heatmap 저장 실패 | `500 HEATMAP_STORAGE_FAILED` |
| DB 저장 실패 | 생성한 heatmap 파일 삭제 |
| API 처리 시간 | 3초 이내 |
...
| Service | 진료기록·X-ray 확인, 기존 결과 재사용, 추론 호출, heatmap DB 경로 변환 및 실패 시 파일 정리 |
| Repository | 기존 결과 조회, 목록·전체 개수 조회, 신규 결과 저장 |
| Model | DB 컬럼, FK, 복합 유일 제약 및 relationship 정의 |
| Worker/Inference | 이미지 전처리, 폐렴 추론, 마지막 Conv layer 기준 Grad-CAM 생성·파일 저장 및 응답 URL 반환 |
...
- [ ] 마지막 Conv layer 기준 Grad-CAM heatmap이 생성된다.
- [ ] heatmap이 `uploads/heatmaps/{record_id}/{uuid}.png`에 저장된다.
- [ ] DB에는 `heatmaps/{record_id}/{uuid}.png` 상대 경로가 저장된다.
- [ ] API는 `/uploads/heatmaps/{record_id}/{uuid}.png` URL을 반환한다.
- [ ] 기존 결과 재사용 시 Grad-CAM을 다시 생성하지 않는다.
- [ ] DB 저장 실패 시 생성된 heatmap 파일이 삭제된다.
- [ ] 결과 목록이 최신순으로 조회된다.
...
- 모델 추론 함수: `predict_pneumonia(image_path: str | Path, record_id: int) -> dict`
- 추론 실행: `asyncio.to_thread()`
- 예측 API 전체 timeout: 3초
